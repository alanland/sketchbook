...
public void setup() {
  ...
  VParticle particleC = new VParticle(width - 250, height * .3f);
  particleC.addBehavior(new BWander(1, 1, 1));



  //create a ParticleString: input: VPhysics, particleA, particleB, resolution, strength
  string = new VParticleString(physics, particleA, particleB, .1f, .2f);
  physics.addGroup(string);
  string = new VParticleString(physics, particleB, particleC, .1f, .2f);
  physics.addGroup(string);
}
