...
int DEFAULTEXHIBITWIDTH = displayWidth;
int DEFAULTEXHIBITHEIGHT = displayHeight;
...
//    ((PGraphicsOpenGL)g).gl.setSwapInterval(1);
  ...
  if(CUSTOMFONT.equals("") || CUSTOMFONT.equals("\"\"")){
    f = new RFont(FONT,372,RFont.LEFT);
  }else{
    try{
      f = new RFont(CUSTOMFONT,372,RFont.LEFT);
    }catch(Exception e){
      f = new RFont(FONT,372,RFont.LEFT);
    }
  }
